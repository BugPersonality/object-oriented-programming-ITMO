...
class PointCleaner{
    ...
    private func checkByCount(points: [RestorePoint], maxCount: Int) -> Bool{
        return points.count > maxCount
    }
    
    private func checkBySize(points: [RestorePoint], maxSize: Double) -> Bool{
    ...
    private func checkBydate(points: [RestorePoint], maxDate: String) -> Bool{
        for point in points{
    ...
    private func clearByCount(points: [RestorePoint], maxCount: Int) -> [RestorePoint]{
        var newPoints = points
        ...
        if lastIdex == 0 && newPoints[lastIdex + 1] is FullRestorePoint{
            newPoints.removeFirst()
            return newPoints
        }
        
        if lastIdex == 0 && newPoints[lastIdex + 1] is IncrementalRestorePoint{
            return newPoints
        }
        
        if lastIdex == 1 && newPoints[lastIdex] is IncrementalRestorePoint{
            return newPoints
        }
        
        if newPoints[lastIdex] is FullRestorePoint{
            for _ in 0..<lastIdex{
                newPoints.removeFirst()
            }
            return newPoints
            ...
            if newPoints[lastIdex]is IncrementalRestorePoint{ // fiiiiiiiii...
                return newPoints
            }
            else{
                for _ in 0..<lastIdex{
...
//        return newPoints
    ...
    private func clearBySize(points: [RestorePoint], maxSize: Double) -> [RestorePoint]{
        var newPoints = points
...
//        return newPoints
    ...
    private func clearByDate(points: [RestorePoint], maxDate: String) -> [RestorePoint]{
        var newPoints = points
        for i in (0..<newPoints.count).reversed(){
    ...
    private func checkDateOnCorrect(date: String) -> Bool{
